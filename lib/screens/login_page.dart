...
class _LoginPageState extends State<LoginPage> {
  ...
  // (your old commented handleLogin versions remain unchanged above ⬆️)
  ...
  void handleLogin() {
    ...
    if (selectedRole == 'Student') {
      ...
      if (valid) {
        ...
        // ✅ Also send to backend with teacherId
        ApiService.addTeacher({
          "name": teacherNameController.text,
          "department": teacherDeptController.text,
          ...
          if (response["success"] == true || response["message"] != null) {
            ...
            ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(
                content: Text(response["message"] ?? "Failed to add teacher"),
              ),
            );
          }
        }).catchError((e) {
  ...
  // (your role selection + forms remain unchanged ⬇️)
   Widget _roleSelectionCard(String title, IconData icon, Color color) {
    return RoleCard(
      title: title,
      subtitle: 'Continue as $title',
